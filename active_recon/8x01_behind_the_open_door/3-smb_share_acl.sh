...
SHARE=$(smbmap -H "$TARGET" -u "" -p "" 2>/dev/null | awk '/READ/ {print $1}' | grep -Ev '^(print\$|IPC\$|ADMIN\$|C\$)$' | head -n 1)

# 2. Fallback: If smbmap fails or requires different syntax, try without explicit null creds.
if [ -z "$SHARE" ]; then
    SHARE=$(smbmap -H "$TARGET" 2>/dev/null | awk '/READ/ {print $1}' | grep -Ev '^(print\$|IPC\$|ADMIN\$|C\$)$' | head -n 1)
fi
...
if [ -z "$SHARE" ]; then
    SHARE=$(netexec smb "$TARGET" -u "" -p "" --shares 2>/dev/null | awk '/READ/ {for(i=1;i<=NF;i++) if($i~/READ/) print $(i-1)}' | grep -Ev '^(print\$|IPC\$|ADMIN\$|C\$)$' | head -n 1)
fi
